...
const langs = {
  "zh-CN": {
    ...
    prompt: {
      ...
      definition: "SuperCat 特有功能：一个 `.d.ts` 文件的引用地址，能够自动补全编辑器的提示",
...
* once 13 * * 每个月 13 号的每小时运行一次`.replace(/\n/g, "<br>"),
  ...
  "en-US": {
    ...
    prompt: {
      ...
      definition: "SuperCat-only: URL of a `.d.ts` file used for editor auto-completion",
...
* once 13 * * Run once every hour on the 13th day of each month`.replace(/\n/g, "<br>"),
  ...
  "zh-TW": {
    ...
    prompt: {
      ...
      definition: "SuperCat 特有功能：一個 `.d.ts` 檔案的引用網址，可啟用編輯器自動提示",
...
* once 13 * * 每月 13 號的每小時執行一次`.replace(/\n/g, "<br>"),
  ...
  "ja-JP": {
    ...
    prompt: {
      ...
      definition: "SuperCat 専用機能：`.d.ts` ファイルの URL。エディタの補完を有効にします。",
...
* once 13 * * 毎月 13 日の各時間帯で 1 回実行`.replace(/\n/g, "<br>"),
  ...
  "de-DE": {
    ...
    prompt: {
      ...
      definition: "Nur für SuperCat: URL zu einer `.d.ts`-Datei für Editor-Autovervollständigung",
...
* once 13 * * Einmal stündlich am 13. Tag jedes Monats ausführen`.replace(/\n/g, "<br>"),
  ...
  "vi-VN": {
    ...
    prompt: {
      ...
      definition: "Tính năng riêng của SuperCat: URL tới tệp `.d.ts` giúp bật gợi ý tự động trong trình soạn thảo",
...
* once 13 * * Chạy 1 lần mỗi giờ vào ngày 13 hằng tháng`.replace(/\n/g, "<br>"),
  ...
  "ru-RU": {
    ...
    prompt: {
      ...
      definition: "Особенность SuperCat: URL файла `.d.ts`, используемого для автодополнения в редакторе",
