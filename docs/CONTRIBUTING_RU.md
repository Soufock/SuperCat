...
# Руководство по внесению вклада в SuperCat

Мы очень благодарны за ваш вклад в развитие SuperCat! Это руководство призвано помочь вам вносить вклад в SuperCat более стандартизированным образом, поэтому, пожалуйста, обязательно внимательно его прочитайте.

## Создание Issue

Перед созданием Issue мы рекомендуем сначала проверить [существующие Issues](https://github.com/Soufock/supercat/issues), чтобы избежать дублирования.

### Сообщение о проблемах, сбоях и уязвимостях

SuperCat — это постоянно развивающийся проект. Если вы столкнетесь с проблемами в процессе использования и уверены, что они вызваны SuperCat, вы можете создать Issue. При создании, приложите подробные шаги для воспроизведения и информацию о среде выполнения.
...
SuperCat использует [pnpm](https://pnpm.io/) для управления зависимостями проекта. Если у вас уже установлен npm, следующие команды помогут вам быстро настроить среду разработки:
...
Ветка `main` является основной веткой SuperCat. Для сохранения целостности кода, не изменяйте напрямую ветку `main`.
...
Если вы не являетесь участником команды SuperCat, вы можете сначала сделать fork этого репозитория, а затем создать Pull Request
...
Документация SuperCat находится в другом репозитории: [scriptcat.org](https://docs.scriptcat.org), она написана с использованием [docusaurus](https://docusaurus.io/), вот некоторые
...
с переводом контента, связанного с SuperCat, вы можете найти проект SuperCat на Crowdin и начать работу по переводу.

- `src/locales` — каталог файлов перевода для [расширения](https://github.com/Soufock/supercat)
...
SuperCat использует ESLint для стандартизации стиля кода и Vitest для модульного тестирования. Вы можете использовать следующие команды для запуска:
...
Для разработки страниц SuperCat используются следующие технологии:
...
Если вы хотите запустить SuperCat локально, вы можете использовать следующую команду:
